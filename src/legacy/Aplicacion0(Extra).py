...
class ResponsiveMenuLayout(Row):
    ...
    def _navigation_change(self, e):
        self._change_displayed_page() #cambia el contenido de la pagina del menu seleccionada
        self.check_toggle_on_select() #cambia en el menu la opcion seleccionada
        self.page.update()
...
def menu(page: Page, title="Sistema de Seguridad"):
    page.clean()
    page.title = title

    menu_button = IconButton(icons.MENU) #barra principal para el boton de menu (tres rallitas)
    barra_menu = AppBar(
        leading=menu_button,
        leading_width=40,
    ...
    pages = [
        ...
        (
            ...
            create_page(
                "Escanee el codigo de la puerta",
    ...
    menu_layout.navigation_rail.leading = ElevatedButton(
        "Cerrar Sesion", icon=icons.ADD, expand=True, on_click=lambda e: cerrar_session()
    )
